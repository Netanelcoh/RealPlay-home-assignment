...
- **Score is the raw sum of `amount`**, in integer cents. Ties are left undefined.
...
- **The snapshot boundary.** An in-window bet arriving after the snapshot gets a ledger row and a warning, but can't move frozen placements. The transaction doesn't help — under `READ COMMITTED` the snapshot takes no locks. Closing it needs `FOR SHARE`/`FOR UPDATE` on the `Tournament` row plus a grace period on the job delay.
